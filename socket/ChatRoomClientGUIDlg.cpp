...
BEGIN_MESSAGE_MAP(CChatRoomClientGUIDlg, CDialog)
	...
	//ON_NOTIFY(IPN_FIELDCHANGED, IDC_IPADDRESS, &CChatRoomClientGUIDlg::OnIpnFieldchangedIpaddress)
	ON_BN_CLICKED(IDC_CONN2, &CChatRoomClientGUIDlg::OnBnClickedConn2)
END_MESSAGE_MAP()
...
void CChatRoomClientGUIDlg::OnBnClickedConn()
{
	...
	//创建服务器（多线程）
	string IP(ip);
	this->myserver = new Server(IP, this->m_Port, this->addr);

	HANDLE thread2 = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)(run), (LPVOID)(myserver), 0, 0);
	this->mythread = thread2;
}

void CChatRoomClientGUIDlg::OnBnClickedConn2()//关闭服务器按钮
{
	if (this->mythread)
	{
		TerminateThread(this->mythread, 0);
		string msg("已回收己方服务器！");
...
void run(Server *server) {
	if (server->WinsockStartup() == -1) return;
	if (server->ServerStartup() == -1)return;
	if (server->ListenStartup() == -1)return;
	string msg;
	msg = "已开启服务器！ IP:" + server->ip+ "     Port:" + to_string(server->port);
	CChatRoomClientGUIDlg *dlg1 = (CChatRoomClientGUIDlg *)(AfxGetApp()->GetMainWnd());
	dlg1->m_List.AddString(CString(msg.c_str()));
	msg = "主目录为：" + server->addr;
	CChatRoomClientGUIDlg *dlg2 = (CChatRoomClientGUIDlg *)(AfxGetApp()->GetMainWnd());
	dlg2->m_List.AddString(CString(msg.c_str()));
