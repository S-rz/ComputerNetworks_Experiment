...
void TCPReceiver::receive(Packet &packet) {
	...
	if (checkSum == packet.checksum && this->expectSequenceNumberRcvd == packet.seqnum) {
		...
		cout << "接收方期待报文的seqnum=" << (this->lastAckPkt.acknum + 1) % 4 << endl;
		pUtils->printPacket("接收方重新发送上次报文", lastAckPkt);
		pns->sendToNetworkLayer(SENDER, lastAckPkt);	//调用模拟网络环境的sendToNetworkLayer，通过网络层发送期待的报文
	}
}
